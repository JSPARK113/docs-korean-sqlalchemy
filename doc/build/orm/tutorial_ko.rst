...
그 클래스의 인스턴스를 대응하는 테이블의 행에 결합시키는 메서드를 제공한다.
...
것을 의미하며 그 아래의 텍스트는 예상되는 반환 값을 의미한다.

버전 확인
=============

최소한 **1.2 버전**\ 의 SQLAlchemy를 사용하고 있는지 확인한다.::
...
이 튜토리얼에서는 in-memory-only SQLite 데이터베이스를 사용한다. 연결에는
...
ORM을 사용할 하면 최조의 설정 과정은
데이터베이스 테이블을 구성하고 그 테이블에 매핑될 우리의 클래스를 정의하는 것이다.
SQLAlchemy에서 이 두 작업은 :ref:`declarative_toplevel`\ 로 알려진 시스템을 사용하여 같이 수행한다.
이 시스템을 쓰면 매핑할 실제 데이터베이스 테이블을 구성하는 명령어를 포함하는 클래스를 만들 수 있다.

선언(Declarative) 시스템으로 클래스를 매핑할 때는
테이블과와 클래스의 카탈로그를 관리하는 베이스 클래스를 이용한다.
이 베이스 클래스는 **declarative base class**\ 로 알려져 있다.
...
이 클래스에 어플리케이션에서 필요한 다른 메서드와 속성을 정의할 수 있다.
...
선언 시스템을 쓴다면 선언용 베이스 클래스의 ``.metadata`` 속성으로
이 객체에 접근할 수 있다.
